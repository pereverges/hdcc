...
void set_load_train_data(char* path){
    train_data = (struct DataReader *)calloc(1, sizeof(struct DataReader));
    train_data -> path = path;
    train_data -> fp = fopen(path, "r");
    train_data -> line = NULL;
    train_data -> len = 0;

    if (train_data -> fp == NULL)
        exit(EXIT_FAILURE);
}

void set_load_train_labels(char* path){
    train_labels = (struct DataReader *)calloc(1, sizeof(struct DataReader));
    train_labels -> path = path;
    train_labels -> fp = fopen(path, "r");
    train_labels -> line = NULL;
    train_labels -> len = 0;

    if (train_labels -> fp == NULL)
        exit(EXIT_FAILURE);
}
...
void train_loop(){
    ...
    for(i = 0; i < floor(TRAIN/b); i++){
        ...
        int train_l[b];
        for (j = 0; j < b; j++){
            train_d[j] = load_train_data_next_line();
            train_l[j] = load_train_labels_next_line();
...
float test_loop(){
    ...
    map_range_clamp(TEST_DATA,TEST,POSITION_DIM-1, res);
    int correct_pred = 0;
    for(i = 0; i < TEST; i++){
...
int main(int argc, char **argv) {
    ...
    weights();
    set_load_train_data(argv[1]);
    set_load_train_labels(argv[2]);
